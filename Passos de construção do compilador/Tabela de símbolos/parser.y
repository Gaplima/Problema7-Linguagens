...
program:
    stmt_list
    {
        ...
        print_symbol_table(); // Imprime a tabela ao final
  ...
  | ID ASSIGN expr SEMI
    {
        ...
        if (lookup_symbol($1) == NULL) {
            // Se não existe, cria. 
            // Nota: Como não temos declaração explicita (ex: int x), 
            // vou assumir um tipo genérico (0 ou TYPE_INT) por enquanto.
            install_symbol($1, 0); 
        } else {
            // Opcional: Checagem de tipos poderia ocorrer aqui
            // printf("Variavel %s ja existe, atualizando valor...\n", $1);
        }

        // Cria nó da AST
        $$ = create_assign($1, $3);
        
        // IMPORTANTE: install_symbol faz strdup. 
        // O create_assign também faz strdup.
        // Logo, podemos liberar a string que veio do lexer ($1).
        free($1); 
    }
  | SEMI
    {
...
expr:
  ...
  | ID {
        // Na expressão, verificamos se a variável foi declarada antes de usar
        if (lookup_symbol($1) == NULL) {
            printf("ERRO SEMANTICO: Variavel '%s' usada sem ser declarada!\n", $1);
            // Dependendo da rigidez, você pode chamar exit(1);
        }
        $$ = create_var($1);
        free($1);
    }
