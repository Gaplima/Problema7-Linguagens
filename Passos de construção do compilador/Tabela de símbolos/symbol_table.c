...
void install_symbol(char *name, int type) {
    ...
    newSym->name = strdup(name); // Importante: Duplicar a string para ter propriedade dela
    newSym->type = type;
    
    // Inserção no início da lista encadeada (mais rápido)
    newSym->next = symbolTable[idx];
    symbolTable[idx] = newSym;
    
    printf("[DEBUG] Simbolo '%s' instalado na posicao %d.\n", name, idx);
}
